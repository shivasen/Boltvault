...
const createMediaElement = (mediaItem) => {
    ...
    return `<img src="${mediaItem.url}" alt="${mediaItem.name}" class="max-h-[80vh] w-auto h-auto object-contain">`;
};
            ...
            <div id="media-viewer-modal" class="modal-container fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 p-4 animate-fade-in-up">
                <div class="flex flex-col lg:flex-row w-full h-full max-w-screen-xl max-h-screen">
                    <div class="flex-1 flex items-center justify-center bg-black relative">
                        ...
                        <button data-action="close-modal" class="absolute top-4 right-4 text-white bg-black/50 rounded-full p-2 hover:bg-black/80 transition-colors">
                    ...
                    <div class="w-full lg:w-80 bg-surface text-on-surface p-6 flex-shrink-0 overflow-y-auto">
                        <div class="flex items-center mb-4">
                            ${character.profile_picture_url 
                                ? `<img src="${character.profile_picture_url}" class="w-12 h-12 rounded-full object-cover mr-4">`
                        ...
                        <div class="border-t border-gray-700 pt-4 space-y-2">
                             <button data-action="show-edit-media-modal" data-id="${mediaItem.id}" class="w-full flex items-center justify-center text-on-surface bg-surface hover:bg-primary hover:text-background font-semibold py-2 px-4 rounded-md transition-colors border border-on-surface/20">
                                <div class="w-5 h-5 mr-2">${Edit}</div>
                                Edit Post
                            </button>
                             <button data-action="delete-media" data-id="${mediaItem.id}" class="w-full flex items-center justify-center text-red-400 bg-red-500/10 hover:bg-red-500/20 font-semibold py-2 px-4 rounded-md transition-colors">
