...
export function renderGallery(container, media, characters, filters, searchTerm, options = {}) {
  const { isLoadMore = false, hasMore = false } = options;
  ...
  if (isLoadMore) {
    ...
    if (grid && media.length > 0) {
      grid.insertAdjacentHTML('beforeend', media.map(item => createMediaCard(item, characters)).join(''));
    }
  ...
  const galleryHtml = `
    <header class="mb-6">
      <div class="flex flex-col sm:flex-row justify-between items-center mb-4">
        ...
        <div class="flex items-center space-x-2">
          <button data-action="show-filter-modal" aria-label="Open filter and sort options" class="flex items-center bg-surface px-4 py-2 rounded-lg text-on-surface hover:bg-primary hover:text-background transition-colors">
    ...
    <div id="media-grid" class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 sm:gap-4">
      ${media && media.length > 0 ? media.map(item => createMediaCard(item, characters)).join('') : renderEmptyState(isLoggedIn, hasFiltersOrSearch)}
    </div>
    <div id="load-more-container" class="text-center w-full col-span-full">
        ${hasMore ? `<button id="load-more-btn" data-action="load-more-media" class="mt-8 bg-surface text-on-surface font-bold py-2 px-6 rounded-md hover:bg-primary hover:text-background transition h-10">Load More</button>` : ''}
    </div>
  `;
  container.innerHTML = galleryHtml;

  addVideoHoverListeners(container);
}
